...
class VoxelSampler(Sampler):
    ...
    pX0[t+1] is provided as a template input to the model at time t
    """
    # TODO: move com to voxel least sdf point. sample_init?
    def sample_init(self, return_forward_trajectory=False):
        """
        Initial features to start the sampling process.
        ...
        # self.target_feats = iu.process_target(self.inf_conf.input_pdb, parse_hetatom=True, center=True, voxel_center=np.array([7.081, 18.797, -6.830]))
        self.target_feats = iu.process_target(self.inf_conf.input_pdb, parse_hetatom=True, center=True)
        # print("🧑‍💻")
        # print(self.target_feats.keys())
        # print(self.target_feats['xyz_27'].shape)
        # self._to_pdb(self.target_feats['xyz_27'],'/home/annung202/xyz_27.pdb')
        # self._to_pdb(self.target_feats['xyz_27']-torch.tensor([7.081, 18.797, -6.830]),'/home/annung202/xyz_27_moved.pdb')
        # # self.target_feats['xyz_27'] -= torch.tensor([7.081, 18.797, -6.830])
        # kkk
        ################################
        ### Generate specific contig ###
        ################################
